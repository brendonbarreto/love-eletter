...
import { getSpy, getPrincess, is, Character, getCards } from '../cards'
...
describe('deck', () => {
  ...
  describe('getNewDeck', () => {
    ...
    it.each(getCards().map(card => [card.copies, card.character]))('should include %p copies of %p', (copies, character) => {
      const deck = getNewDeck()
